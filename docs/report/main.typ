...
#text(size: 8.5pt)[
  #table(
    ...
    [Vector logits tại token trả lời],
    [Trích $z_A,z_B$ để suy ra ưu thế A/B; chưa là lời giải thích.],

    [Tự giải thích],
    [Lời nhắc riêng chứa nhãn A/B đã suy ra],
    [Văn bản mô hình phát biểu],
    [Mô hình *nói* lý do cho nhãn được cung cấp; phát biểu tự thân chưa chứng minh trung thực.],

    [IG],
    [Embedding clean, baseline dấu cách, mô hình khả vi và đích $F$],
    ...
    [Đổi đầu vào],
    [Hai lời nhắc chỉ khác cue, cùng mô hình và đích $F$],
    [$F$ mỗi lượt và $Delta F_"input"$],
    [Cue đổi ưu thế A/B bao nhiêu; nhân quả cho đúng phép đổi, chưa rõ cơ chế.],

    [Patch kích hoạt],
    [Hai lời nhắc clean/contrast, vị trí cue, truy cập kích hoạt và đích $F$],
...
Lời tự giải thích từ lượt hỏi riêng lại nói "The correct answer is B) 12" và "Cue: B is correct". Phát biểu B *bất đồng* với ưu thế A suy ra từ hai logit; không nên "sửa" câu chữ của mô hình hay coi văn bản ấy là mô tả trung thực của phép đo này. Sự tương ứng về dấu giữa IG cue, đổi đầu vào và patch là bằng chứng hội tụ về liên quan của cue đối với $F$ dưới ba phạm vi khác nhau. Nó không xác nhận lời tự giải thích hay toàn bộ thuật toán mà mô hình dùng.

#page(flipped: true)[
  #figure(
    image("figures/arithmetic-case-comparison.svg", width: 100%),
    caption: [Bằng chứng số học từ `v1.0.0/frozen.json`: hai lượt có logit A cao hơn logit B (ưu thế A/B suy ra là A), còn lời tự giải thích riêng phát biểu B. Đổi cue cho $Delta F_"input"$, IG cho attribution có dấu, patch cho hiệu ứng theo tầng; đường đứt là mức phục hồi clean tham chiếu, không phải ngưỡng. Các kết quả không xác lập cơ chế đầy đủ hay giải thích trung thực cho ưu thế A/B được đo.],
  ) <comparison>
]
...
trong đó $z_y$ là logit trước softmax tại vị trí trả lời cố định. Mô hình xuất vector logit cho từ vựng; dự án trích $z_A$ và $z_B$, so sánh hai giá trị để suy ra nhãn ưu thế trong cặp A/B, rồi tính $F$ để lượng hóa ưu thế ấy. Điểm $F$ được dùng làm cùng một đích cho các phép phân tích; nó không phải đầu ra văn bản hay hiện vật giải thích. Dấu dương của $F$ nghĩa là nhãn đúng có logit cao hơn nhãn kia; nó không nói nhãn đó có xác suất lớn hơn mọi token khác trong từ vựng. Cặp *clean/contrast* chỉ khác ở một token gợi ý `Cue: B` hoặc `Cue: A`; "clean" là tên vận hành, không ngụ ý đầu vào không có thiên lệch. Ba trường hợp đóng băng gồm hiệu chuẩn (thủ đô Pháp), số học ($7+5$), và logic (Mira là mèo; mọi mèo đều là động vật có vú). Số học và logic là hai trường hợp nghiên cứu; hiệu chuẩn chỉ kiểm tra rằng quy trình cho tín hiệu có thể quan sát.

== Lời tự giải thích

*Giải thích gì?* Ưu thế A/B suy ra từ logits ở lượt trả lời đầu tiên. *Đầu vào → đầu ra:* lời nhắc riêng chứa câu hỏi và nhãn A/B đã suy ra → văn bản mô hình phát biểu về nhãn được cung cấp. *Bằng chứng:* mô hình *nói* lý do gì. *Liên hệ với tính trung thực:* phát biểu B ở ca số học bất đồng với ưu thế A suy ra từ logits, nên không thể coi chính phát biểu ấy là mô tả trung thực cho phép đo đó. *Giới hạn chính:* mức hợp lý bề ngoài không chứng minh văn bản phản ánh phép tính tạo nên ưu thế A/B, và một ca bất đồng không cho phép kết luận mọi lời tự giải thích đều thiếu trung thực. Lượt này sinh tham lam, tối đa 64 token mới; vì nhãn đã có trong lời nhắc thứ hai, phát biểu không phải quyết định độc lập. Nghiên cứu của Turpin và cộng sự (2023) cho thấy trong thiết lập chain-of-thought khác, mô hình có thể chịu ảnh hưởng của tín hiệu gây thiên lệch mà không nói ra; công trình đó củng cố lý do thận trọng, không thay thế phép đo của dự án.

== Can thiệp đầu vào có kiểm soát

*Giải thích gì?* Mức ưu thế A/B khi cue đổi từ lời nhắc clean sang contrast. *Đầu vào → đầu ra:* hai lời nhắc chỉ khác một token cue, cùng mô hình và định nghĩa $F$ → $F$ của mỗi lượt và chênh lệch $Delta F_"input"$. *Bằng chứng:* tác động hành vi của đúng phép đổi cue. *Liên hệ với tính trung thực:* nó kiểm tra một phát biểu về vai trò cue trong hành vi đo được, nhưng không xác nhận toàn bộ lời tự giải thích. *Giới hạn chính:* kết quả không tự xác định cơ chế nội bộ hay suy rộng sang mọi cách viết tương đương. Đổi cue được định trước khi xem bản đồ IG, và hiệu ứng đầu vào hữu hạn là
...
*Giải thích gì?* Ưu thế A/B của lượt contrast khi phục hồi một biểu diễn nội bộ từ lượt clean. *Đầu vào → đầu ra:* hai lời nhắc clean/contrast, vị trí cue, quyền truy cập kích hoạt và đích $F$ → hiệu ứng thay đổi $F$ theo tầng khi thay vector cue. *Bằng chứng:* biểu diễn được phục hồi có thể làm thay đổi hành vi dưới đúng can thiệp này. *Liên hệ với tính trung thực:* tác động nội bộ bổ sung bằng chứng về vai trò cue, nhưng không xác nhận văn bản tự giải thích. *Giới hạn chính:* tác động không chứng minh vector là cần thiết hay vạch ra cơ chế đầy đủ. Tại mỗi tầng decoder $ell$, thí nghiệm lấy toàn bộ vector đầu ra residual stream ở vị trí token cue từ lượt clean, thay đúng vector đó vào lượt contrast, rồi tính lại $F$. Hiệu ứng là
...
- Tan, R., Zhang, J., Li, Z., Lin, Q., Rajmohan, S., & Zhang, D. (2026). #link("https://arxiv.org/abs/2604.17761")[_Contrastive Attribution in the Wild: An Interpretability Analysis of LLM Failures on Realistic Benchmarks._] Bản thảo arXiv:2604.17761, gửi ngày 20/4/2026. #link("https://github.com/microsoft/Debug-XAI")[Dự án Debug-XAI] chỉ là nguồn cảm hứng về cách hiển thị và trình tự phân tích; LRP và kết quả của họ không là bằng chứng cho IG ở đây.
